...
| | 使う人の手間 | 費用 | 通信 | 難しいフォーム |
| --- | --- | --- | --- | --- |
| **A ブラウザ内蔵のAI**（既定） | **ゼロ。入れたらすぐ** | **0円** | **なし**（端末の中だけ） | △ 小さいモデル |
| **B Claude**（APIキー） | キーを発行して貼る | 1回5〜8円 | Anthropic へ | ◎ |

既定は**自動**で、内蔵AIが使えればA、使えなければBに倒れます。設定画面で固定もできます。

A は Chrome 138 以降の `LanguageModel`（Gemini Nano）。**空き容量22GB・メモリ16GB以上**が要り、
配信されていない端末では `availability()` が `unavailable` を返します。その場合は B を使ってください。

A は道具を呼ぶ仕組みが無いので、「欄の一覧」を渡して「埋める値の一覧」を JSON で返させ、
**読む → 埋める → 読み直す** を最大3周します（読み直すので「選んだら欄が増える」にも追いつけます）。
...
BRAIN=claude npm run rec:001     # 頭脳を固定（local / claude / auto）
PASS2=1 npm run rec:001          # 「人が送信 → エラー → AIが直す」まで撮る
```

> **頭脳A は自動では撮れません。** 内蔵AIがあるのは Chrome だけですが、その Chrome が
> コマンドラインからの拡張機能の読み込みを拒否します。A を見せたい場合は、
> Chrome に手で読み込んで、画面録画ソフトで撮ってください。
